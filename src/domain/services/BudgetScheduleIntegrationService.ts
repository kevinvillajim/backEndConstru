...
// ✅ CORRIGIDO: Usar rutas correctas y importar desde las entidades
import {
...
// ✅ CORRIGIDO: Importar los enums desde el modelo de dominio con ruta correcta
import {
	ActivityStatus,
	ActivityType,
	ActivityPriority,
	ConstructionTrade,
} from "../models/calculation/ScheduleActivity";
import { BudgetLineItem, CreateBudgetLineItemDTO, LineItemSource, LineItemType } from "../../domain/models/calculation/BudgetLineItem";
...
export class BudgetScheduleIntegrationService {
	constructor(
		...
		private progressRepository: any // Repositorio de progreso
	) {}

	async synchronizeBudgetAndSchedule(
		...
		try {
			...
			const budgetItems = await this.lineItemRepository.findByBudget(
				syncRequest.budgetId
			);
			const activities = await this.activityRepository.findByScheduleId(
				syncRequest.scheduleId
			);
			...
			const conflicts = this.detectConflicts(
				differences,
				syncRequest.conflictResolution
			);
			...
			switch (syncRequest.syncDirection) {
				case "budget_to_schedule":
					syncResult = await this.syncBudgetToSchedule(
						syncRequest,
						budgetItems,
						activities,
						conflicts
				...
				case "schedule_to_budget":
					syncResult = await this.syncScheduleToBudget(
						syncRequest,
						budgetItems,
						activities,
						conflicts
					...
					break;
				case "bidirectional":
					syncResult = await this.bidirectionalSync(
						syncRequest,
						budgetItems,
						activities,
						conflicts
	...
	async validateCoherence(budgetId: string, scheduleId: string): Promise<any> {
		...
		const activities =
			await this.activityRepository.findByScheduleId(scheduleId);
	...
	private async detectDifferences(
		...
		const itemActivityMap = this.createItemActivityMapping(
			budgetItems,
			activities
		);
		...
		const orphanItems = budgetItems.filter(
			(item) => !itemActivityMap.has(item.id)
		);
		const orphanActivities = activities.filter(
			(activity) => ![...itemActivityMap.values()].includes(activity.id)
		);
		...
		for (const item of budgetItems) {
			try {
				...
				if (activity) {
					...
					const changes = await this.updateActivityFromBudgetItem(
						item,
						activity,
						syncRequest.syncOptions
					);
					if (changes.length > 0) {
						result.summary.scheduleChanges.push({
							activityId: activity.id,
							changes,
						});
						result.syncedItems++;
					...
					const newActivity = await this.createActivityFromBudgetItem(
						item,
						syncRequest.scheduleId
					);
					result.summary.newItemsCreated.push({
						type: "activity",
						id: newActivity.id,
					});
					result.syncedItems++;
				...
				result.errors.push(
					`Error syncing item ${item.id}: ${(error as Error).message}`
				);
		...
		for (const activity of activities) {
			try {
				...
				if (item) {
					...
					const changes = await this.updateBudgetItemFromActivity(
						activity,
						item,
						syncRequest.syncOptions
					);
					...
					const newItem = await this.createBudgetItemFromActivity(
						activity,
						syncRequest.budgetId
					);
					result.summary.newItemsCreated.push({
				...
				result.errors.push(
					`Error syncing activity ${activity.id}: ${(error as Error).message}`
				);
				result.success = false;
			}
		}
	...
	private async bidirectionalSync(
		...
		const budgetToSchedule = await this.syncBudgetToSchedule(
			syncRequest,
			budgetItems,
			activities,
			conflicts
		);
		const scheduleToBudget = await this.syncScheduleToBudget(
			syncRequest,
			budgetItems,
			activities,
			conflicts
		...
		return {
			...
			conflictsDetected: Math.max(
				budgetToSchedule.conflictsDetected,
				scheduleToBudget.conflictsDetected
			),
			conflictsResolved:
				budgetToSchedule.conflictsResolved + scheduleToBudget.conflictsResolved,
			warnings: [...budgetToSchedule.warnings, ...scheduleToBudget.warnings],
			errors: [...budgetToSchedule.errors, ...scheduleToBudget.errors],
			summary: {
				budgetChanges: [
					...budgetToSchedule.summary.budgetChanges,
					...scheduleToBudget.summary.budgetChanges,
				],
				scheduleChanges: [
					...budgetToSchedule.summary.scheduleChanges,
					...scheduleToBudget.summary.scheduleChanges,
				],
				newItemsCreated: [
					...budgetToSchedule.summary.newItemsCreated,
					...scheduleToBudget.summary.newItemsCreated,
				],
			},
		};
	}

	private createItemActivityMapping(
		budgetItems: any[],
		activities: any[]
	): Map<string, string> {
		...
		for (const item of budgetItems) {
			if (!mapping.has(item.id)) {
				const matchingActivity = this.findBestMatch(
					item.description,
					activities
				);
				if (matchingActivity) {
	...
	private compareItemAndActivity(item: any, activity: any): string[] {
		...
		// Comparar cantidades
		if (
			Math.abs(item.quantity - activity.workQuantities.plannedQuantity) > 0.01
		) {
	...
	private findMatchingActivity(item: any, activities: any[]): any {
		...
		if (item.customFields?.linkedActivityId) {
			return activities.find(
				(a) => a.id === item.customFields.linkedActivityId
			);
		}
	...
	private findMatchingBudgetItem(activity: any, budgetItems: any[]): any {
		...
		if (activity.customFields?.budgetLineItemId) {
			return budgetItems.find(
				(i) => i.id === activity.customFields.budgetLineItemId
			);
		}
	...
	private async updateActivityFromBudgetItem(
		item: any,
		activity: any,
		syncOptions: any
	): Promise<string[]> {
		...
		if (
			syncOptions.syncCosts &&
			Math.abs(item.totalCost - activity.plannedTotalCost) > 0.01
		) {
	...
	private async updateBudgetItemFromActivity(
		activity: any,
		item: BudgetLineItem,
		syncOptions: any
	): Promise<string[]> {
		...
		if (
			syncOptions.syncCosts &&
			Math.abs(activity.plannedTotalCost - item.subtotal) > 0.01
		) {
			item.subtotal = activity.plannedTotalCost;
			changes.push("cost_updated");
		}

		if (
			syncOptions.syncQuantities &&
			Math.abs(activity.workQuantities.plannedQuantity - item.quantity) > 0.01
		) {
			item.quantity = activity.workQuantities.plannedQuantity;
			item.unitOfMeasure = activity.workQuantities.unit;
	...
	private async createProgressRecord(
		request: DailyProgressRequest,
		activity: any
	): Promise<ActivityProgressEntity> {
		...
		progressRecord.workCompleted = {
			...
			qualityLevel: "SATISFACTORY" as any,
		};

		// Recursos utilizados
		progressRecord.actualWorkersOnSite = request.actualWorkersOnSite;
		progressRecord.actualHoursWorked = request.actualHoursWorked;
		progressRecord.plannedHoursForDay = 8;

		// Métricas calculadas
		progressRecord.productivityRate = this.calculateProductivityRate(
			request,
			activity
		);
		progressRecord.efficiencyPercentage = this.calculateEfficiency(
			request,
			activity
		);
		...
		progressRecord.qualityIssues =
			request.qualityIssues?.map((issue, index) => ({
				id: `quality_${Date.now()}_${index}`,
				description: issue.issue,
				severity: issue.severity,
				status: "open" as const,
				reportedBy: request.reportedBy,
				correctionAction: issue.correctionRequired
					? "Required"
					: "Not required",
			})) || [];

		// Incidentes de seguridad
		progressRecord.safetyIncidents =
			request.safetyIncidents?.map((incident, index) => ({
		...
		// Obstáculos
		progressRecord.obstacles =
			request.materialIssues?.map((issue) => ({
				type:
					issue.issue === "delivery_delay"
						? ("material_delay" as const)
						: ("other" as const),
		...
		// Costos del día
		progressRecord.dailyLaborCost =
			request.actualWorkersOnSite * request.actualHoursWorked * 25;
		progressRecord.dailyMaterialCost = 0;
		...
		progressRecord.productivityScore = 8.0;
		progressRecord.overallScore = 0;
	...
	private async createActivityFromBudgetItem(
		item: any,
		scheduleId: string
	): Promise<any> {
	...
	private async createBudgetItemFromActivity(
		activity: any,
		budgetId: string
	): Promise<BudgetLineItem> {
		const newItemDto: CreateBudgetLineItemDTO = {
			calculationBudgetId: budgetId,
			description: activity.name,
			specifications: activity.description,
			itemType: LineItemType.LABOR,
			source: LineItemSource.CALCULATION,
			quantity: activity.workQuantities?.plannedQuantity || 1,
			unitOfMeasure: activity.workQuantities?.unit || "unit",
			unitPrice:
				activity.workQuantities?.plannedQuantity > 0
					? activity.plannedTotalCost / activity.workQuantities.plannedQuantity
					: 0,
			wastePercentage: 5,
			finalQuantity: 0,
			subtotal: 0,
			category: "LABOR",
			subcategory: activity.activityType || "OTHER",
			regionalFactor: 1,
			difficultyFactor: 1,
			priceDate: new Date(),
			priceSource: "generated_from_schedule",
		...
		newItemDto.finalQuantity =
			newItemDto.quantity * (1 + newItemDto.wastePercentage / 100);
		newItemDto.subtotal = newItemDto.finalQuantity * newItemDto.unitPrice;

		return await this.lineItemRepository.create(newItemDto);
	}
	...
	private async generateCoherenceRecommendations(
		...
		if (discrepancies.length > 0) {
			recommendations.push(
				`Se detectaron ${discrepancies.length} discrepancias entre presupuesto y cronograma`
			);
		...
		if (coherenceScore < 80) {
			recommendations.push(
				"Habilitar sincronización automática para mantener coherencia"
			);
		}
	...
	private async updateCoherenceMetrics(
		budget: any,
		schedule: any
	): Promise<void> {
		const now = new Date();
	...
	private calculateProductivityRate(request: any, activity: any): number {
		if (
			!activity.workQuantities?.plannedQuantity ||
			!activity.plannedDurationDays
		) {
			return 1.0;
		}

		const assumedWorkersPerDay = 2;
		const assumedHoursPerDay = 8;
		const totalPersonHours =
			assumedWorkersPerDay * assumedHoursPerDay * activity.plannedDurationDays;

		return totalPersonHours > 0
			? activity.workQuantities.plannedQuantity / totalPersonHours
			: 1.0;
	}
