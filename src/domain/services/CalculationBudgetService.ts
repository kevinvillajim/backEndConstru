...
} from "../models/calculation/CalculationBudget";
...
export interface BudgetGenerationOptions {
	...
	exchangeRate: number;
	customization?: any;
	projectType?: string; // Add projectType to options
}
...
export class CalculationBudgetService {
	...
	async generateFromCalculationResult(
		...
		const materialsSubtotal =
			this.calculateMaterialsSubtotal(calculationResult);
		...
		const laborSubtotal = options.includeLabor
			? this.calculateLaborCosts(
					calculationResult,
					template,
					options.geographicalZone
				)
			: 0;

		// 4. Calcular costos indirectos
		const indirectCosts = options.includeIndirectCosts
			? this.calculateIndirectCosts(
					adjustedMaterialsSubtotal,
					laborSubtotal,
					template
				)
			: 0;
		...
		const professionalCostsTotal = options.includeProfessionalFees
			? this.calculateProfessionalFees(
					adjustedMaterialsSubtotal,
					laborSubtotal,
					template
				)
		...
		// 10. Determinar projectType desde options, template o valor por defecto
		const projectType =
			options.projectType || template?.projectType || "GENERAL"; // Valor por defecto

		// 11. Crear el presupuesto
		const budget: CalculationBudget = {
			...
			version: 1,
			projectId,
			projectType, // Add the missing projectType property
			userId,
	...
	async applyTemplateToExistingBudget(
		...
		const adjustedMaterialsSubtotal = this.applyTemplateFactors(
			budget.materialsSubtotal,
			template
		);
		...
		const indirectCosts = template.indirectCosts
			? this.calculateIndirectFromTemplate(
					adjustedMaterialsSubtotal,
					laborSubtotal,
					template
				)
		...
		const professionalCostsTotal = template.professionalFees
			? this.calculateProfessionalFromTemplate(
					adjustedMaterialsSubtotal,
					laborSubtotal,
					template
				)
		...
		// 6. Actualizar presupuesto - actualizar projectType si el template lo tiene
		const updatedBudget: CalculationBudget = {
			...budget,
			budgetTemplateId: template.id,
			projectType: template.projectType || budget.projectType, // Update projectType from template
			materialsSubtotal: adjustedMaterialsSubtotal,
			laborSubtotal,
			indirectCosts,
			contingencyAmount,
	...
	validateBudgetCoherence(budget: CalculationBudget): BudgetValidationResult {
		...
		if (budget.contingencyPercentage < 0 || budget.contingencyPercentage > 50) {
			warnings.push(
				"El porcentaje de contingencia parece estar fuera del rango típico (0-50%)"
			);
		}

		if (budget.taxPercentage < 0 || budget.taxPercentage > 30) {
			warnings.push(
				"El porcentaje de impuestos parece estar fuera del rango típico para Ecuador (0-30%)"
			);
		}
		...
		const calculatedContingency =
			budget.subtotal * (budget.contingencyPercentage / 100);
		if (Math.abs(calculatedContingency - budget.contingencyAmount) > 0.01) {
			errors.push(
				"El monto de contingencia no coincide con el porcentaje aplicado"
			);
		}
		...
		if (budget.indirectCosts === 0) {
			suggestions.push(
				"Considere incluir costos indirectos (administración, herramientas, etc.)"
			);
		}

		if (budget.contingencyPercentage === 0) {
			suggestions.push(
				"Considere incluir un porcentaje de contingencia (típicamente 5-15%)"
			);
		}
	...
	compareBudgetVersions(
		...
		const differences = {
			materialsChange: {
				absolute:
					updatedBudget.materialsSubtotal - originalBudget.materialsSubtotal,
				percentage: this.calculatePercentageChange(
					originalBudget.materialsSubtotal,
					updatedBudget.materialsSubtotal
				),
			},
			laborChange: {
				absolute: updatedBudget.laborSubtotal - originalBudget.laborSubtotal,
				percentage: this.calculatePercentageChange(
					originalBudget.laborSubtotal,
					updatedBudget.laborSubtotal
				),
			},
			indirectChange: {
				absolute: updatedBudget.indirectCosts - originalBudget.indirectCosts,
				percentage: this.calculatePercentageChange(
					originalBudget.indirectCosts,
					updatedBudget.indirectCosts
				),
			},
			totalChange: {
				absolute: updatedBudget.total - originalBudget.total,
				percentage: this.calculatePercentageChange(
					originalBudget.total,
					updatedBudget.total
				),
			},
		};
	...
	private calculateLaborCosts(
		...
		if (!template?.laborRates) {
			...
			return this.calculateStandardLaborCosts(
				calculationResult,
				geographicalZone
			);
		}
	...
	private calculateStandardLaborCosts(
		calculationResult: CalculationResult,
		geographicalZone: string
	): number {
	...
	private calculateTemplatedLaborCosts(
		calculationResult: CalculationResult,
		template: BudgetTemplate
	): number {
		...
		const masterBuilderCost =
			(laborRates.masterBuilder || 35) * estimatedDays * 0.3;
	...
	private calculateProfessionalFees(
		materialsCost: number,
		laborCost: number,
		template: BudgetTemplate | null
	): number {
	...
	private calculateLaborFromTemplate(
		budget: CalculationBudget,
		template: BudgetTemplate
	): number {
