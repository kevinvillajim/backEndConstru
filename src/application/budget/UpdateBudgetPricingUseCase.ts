...
import {
	...
	PriceUpdateResult,
	MaterialPriceChange,
} from "../../domain/services/BudgetPricingService";
import {
	CalculationBudget,
	BudgetCustomization,
...
export class UpdateBudgetPricingUseCase {
	...
	async execute(
		...
		if (materialLineItems.length === 0) {
			throw new Error(
				"No hay materiales en este presupuesto para actualizar precios"
			);
		}

		// 3. Determinar materiales a actualizar
		const materialsToUpdate = this.determineMaterialsToUpdate(
			materialLineItems,
			request
		);

		// 4. Obtener comparaciones de precios
		const priceComparisons = await this.getPriceComparisons(
			materialsToUpdate,
	...
	async comparePricesOnly(
		...
		for (const lineItem of materialLineItems) {
			const materialPrices =
				await this.budgetPricingService.compareMaterialPrices(
					lineItem.materialId!,
					budget.geographicalZone
				);
			...
			if (materialPrices.length > 0) {
				...
				const lowestPrice = Math.min(
					...materialPrices.map((p) => p.currentPrice)
				);
				const mostReliablePrice = materialPrices.sort(
					(a, b) => b.priceSource.reliability - a.priceSource.reliability
				)[0].currentPrice;

				let recommendation: PriceComparisonResult["recommendation"] =
					"KEEP_CURRENT";
				...
				if (lowestPrice < currentPrice * 0.9) {
					...
					mostReliablePrice < currentPrice * 0.95 &&
					mostReliablePrice !== currentPrice
				) {
					recommendation = "UPDATE_TO_MOST_RELIABLE";
					estimatedSavings =
						(currentPrice - mostReliablePrice) * lineItem.quantity;
				} else if (Math.abs(currentPrice - lowestPrice) / currentPrice > 0.15) {
					// Diferencia > 15%
				...
				comparisons.push({
					...
					materialName:
						material?.name || lineItem.description || "Material desconocido",
					currentPrice,
	...
	private async validateAndGetBudget(
		budgetId: string,
		userId: string
	...
	private async getPriceComparisons(
		...
		for (const materialId of materialIds) {
			try {
				const prices = await this.budgetPricingService.compareMaterialPrices(
					materialId,
					geographicalZone
				);
				...
				console.error(
					`Error obteniendo precios para material ${materialId}:`,
					error
				);
			...
			try {
				...
				if (manualAdjustment) {
					...
					const availablePrices =
						priceComparisons.get(lineItem.materialId!) || [];

					if (availablePrices.length > 0) {
						switch (request.updateSource) {
							case "IPCO":
								const ipcoPrice = availablePrices.find(
									(p) => p.priceSource.type === "IPCO"
								);
								if (ipcoPrice) {
							...
							case "SUPPLIER":
								const supplierPrice = availablePrices.find(
									(p) => p.priceSource.type === "SUPPLIER"
								);
								if (supplierPrice) {
							...
							case "ALL":
								...
								const mostReliable = availablePrices.sort(
									(a, b) =>
										b.priceSource.reliability - a.priceSource.reliability
								)[0];
								if (mostReliable) {
				...
				if (newPrice !== lineItem.unitPrice) {
					const changePercentage =
						((newPrice - lineItem.unitPrice) / lineItem.unitPrice) * 100;
					...
					if (Math.abs(changePercentage) > 1 || request.forceUpdate) {
						...
						if (Math.abs(changePercentage) > 5) {
							...
							result.significantChanges.push({
								materialId: lineItem.materialId!,
								materialName:
									material?.name || lineItem.description || "Material",
						...
						// ✅ CORREGIDO: Preparar datos para actualización
						const updatedLineItemData: Partial<BudgetLineItem> = {
							unitPrice: newPrice,
							subtotal: lineItem.quantity * newPrice,
							updatedAt: new Date(),
							metadata: {
								...lineItem.metadata,
						...
						// ✅ CORREGIDO: Usar el repositorio correctamente
						const updatedLineItem = await this.budgetLineItemRepository.update(
							lineItem.id,
							updatedLineItemData
						);

						if (updatedLineItem) {
							// Actualizar el modelo local para cálculos posteriores
				...
				result.errors.push(
					`Error actualizando precio de ${lineItem.materialId}: ${errorMessage}`
				);
			}
		}
	...
	private async recalculateBudgetTotals(
		...
		const newContingencyAmount =
			newSubtotal * (budget.contingencyPercentage / 100);
		...
		try {
			...
			if (priceUpdateResult.updatedCount > 0) {
				const notification = {
					...
					message:
						`Se actualizaron ${priceUpdateResult.updatedCount} materiales en "${budget.name}". ` +
					...
					content:
						`Se actualizaron ${priceUpdateResult.updatedCount} materiales en el presupuesto "${budget.name}". ` +
					...
					priority:
						Math.abs(totalImpact.percentageChange) > 10
							? NotificationPriority.HIGH
							: NotificationPriority.MEDIUM,
					// ✅ CORRIGIDO: Cambiar 'data' por campos requeridos por NotificationEntity
			...
			for (const significantChange of priceUpdateResult.significantChanges) {
				if (Math.abs(significantChange.changePercentage) > 15) {
					const notification = {
						...
						message:
							`${significantChange.materialName}: ` +
						...
						content:
							`Se detectó un cambio significativo en el precio del material ${significantChange.materialName}. ` +
						...
						relatedEntityId: significantChange.materialId,
						isRead: false,
						emailSent: false,
						pushSent: false,
						smsSent: false,
	...
	private generateRecommendations(
		...
		if (totalImpact.percentageChange > 10) {
			recommendations.push(
				"Considere revisar el presupuesto con su cliente debido al aumento significativo de costos"
			);
		} else if (totalImpact.percentageChange < -10) {
			recommendations.push(
				"Aproveche la reducción de costos para mejorar su competitividad"
			);
		}
		...
		if (priceUpdateResult.significantChanges.length > 3) {
			recommendations.push(
				"Configure actualizaciones automáticas semanales debido a la alta volatilidad de precios"
			);
		}

		// Recomendaciones sobre fuentes de precios
		const supplierPricesAvailable = Array.from(priceComparisons.values()).some(
			(prices) => prices.some((p) => p.priceSource.type === "SUPPLIER")
		);

		if (supplierPricesAvailable) {
			recommendations.push(
				"Compare precios con proveedores alternativos para posibles ahorros"
			);
		}
		...
		if (priceUpdateResult.errors.length > 0) {
			recommendations.push(
				"Verifique manualmente los materiales que no pudieron actualizarse"
			);
		}
