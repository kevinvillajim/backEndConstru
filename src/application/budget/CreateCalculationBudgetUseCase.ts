...
import {
	...
	ProfessionalService,
	ComplexityLevel,
} from "../../domain/models/calculation/ProfessionalCost";
...
export interface CreateCalculationBudgetRequest {
	...
	description?: string;
	projectId: string;
	projectType?: string; // ✅ AGREGADO: Campo opcional para el tipo de proyecto
...
export class CreateCalculationBudgetUseCase {
	...
	async execute(
		...
		if (request.budgetTemplateId) {
			template = await this.budgetTemplateRepository.findById(
				request.budgetTemplateId
			);
			if (!template) {
				throw new Error(
					`Template de presupuesto no encontrado: ${request.budgetTemplateId}`
				);
			}
		}
		...
		if (request.calculationResultId) {
			calculationResult = await this.calculationResultRepository.findById(
		...
		if (calculationResult) {
			...
			budget =
				await this.calculationBudgetService.generateFromCalculationResult(
					calculationResult,
					template,
					options,
					userId,
			...
			budget = await this.createManualBudget(
				request,
				template,
				options,
				userId
		...
		const lineItems = await this.createBudgetLineItems(
			savedBudget,
			calculationResult,
			request
		);
		...
		const professionalCosts = await this.createProfessionalCosts(
			savedBudget,
			template,
			options
		);
		...
		const validation =
			this.calculationBudgetService.validateBudgetCoherence(savedBudget);
	...
	private validateRequest(request: CreateCalculationBudgetRequest): void {
		...
		if (
			request.contingencyPercentage < 0 ||
			request.contingencyPercentage > 50
		) {
			throw new Error(
				"El porcentaje de contingencia debe estar entre 0% y 50%"
			);
		}
		...
		if (
			!request.calculationResultId &&
			(!request.customMaterials || request.customMaterials.length === 0)
		) {
			throw new Error(
				"Debe especificar un resultado de cálculo o materiales personalizados"
			);
		}
	}
	...
	private async createManualBudget(
		...
		// ✅ DETERMINAR projectType basado en budgetType o usar valor por defecto
		...
		const budget: CalculationBudget = {
			...
			projectType, // ✅ AGREGADO: Campo requerido
			userId,
	...
	// ✅ NUEVO MÉTODO: Determinar tipo de proyecto
	private determineProjectType(
		budgetType: BudgetType,
		requestProjectType?: string
	): string {
		// Si se proporciona explícitamente, usarlo
		if (requestProjectType && requestProjectType.trim().length > 0) {
			return requestProjectType.trim();
		}

		// Mapear basado en el tipo de presupuesto
		switch (budgetType) {
			case BudgetType.MATERIALS_ONLY:
				return "materiales";
			case BudgetType.LABOR_MATERIALS:
				return "construccion_basica";
			case BudgetType.COMPLETE_PROJECT:
				return "proyecto_completo";
			case BudgetType.PROFESSIONAL_ESTIMATE:
				return "estimacion_profesional";
			default:
				return "general";
		}
	}
		...
		if (request.customMaterials) {
			for (const customMaterial of request.customMaterials) {
				...
				> = {
					...
					unitOfMeasure:
						(material as any)?.unitOfMeasure ||
						(material as any)?.unit ||
						"unidad",
					unitPrice: unitPrice,
				...
				const savedLineItem =
					await this.budgetLineItemRepository.create(lineItemData);
	...
	private async generateRecommendations(
		...
		if (budget.budgetType === BudgetType.MATERIALS_ONLY) {
			recommendations.push(
				"Considere incluir costos de mano de obra para un presupuesto más completo"
			);
		}

		// Recomendaciones basadas en zona geográfica
		if (
			budget.geographicalZone === "ORIENTE" ||
			budget.geographicalZone === "INSULAR"
		) {
			recommendations.push(
				"Considere costos adicionales de transporte para esta zona geográfica"
			);
		}
		...
		if (budget.contingencyPercentage < 5) {
			recommendations.push(
				"Considere aumentar el porcentaje de contingencia (recomendado: 5-15%)"
			);
		}
		...
		if (!template) {
			const templateRecommendations =
				await this.budgetTemplateService.recommendTemplates(
			...
			if (templateRecommendations.length > 0) {
				recommendations.push(
					`Considere usar el template "${templateRecommendations[0].templateName}" para mayor precisión`
				);
			}
		}
	...
	private async updateTemplateUsageStats(templateId: string): Promise<void> {
		try {
			...
			console.log(
				`Actualizando estadísticas de uso para template: ${templateId}`
			);
		} catch (error) {
	...
	private calculateIndirectFromTemplate(
		baseCost: number,
		template: any
	): number {
	...
	private calculateProfessionalFromTemplate(
		baseCost: number,
		template: any
	): number {
		const fees = template.professionalFees;
