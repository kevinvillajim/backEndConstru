...
export class OptimizeProjectScheduleUseCase {
  ...
  async execute(request: OptimizeScheduleRequest): Promise<OptimizeScheduleResponse> {
    ...
    const activityEntities = await this.activityRepository.findByScheduleId(request.scheduleId);
  ...
  // Función para mapear entidades a modelos de dominio
  private mapEntitiesToDomainModels(entities: ScheduleActivityEntity[]): ScheduleActivity[] {
    return entities.map(entity => ({
      ...
      resourceRequirements: this.mapResourceRequirements(entity.resourceAssignments || []),
  ...
  private hasPredecessorRelationship(activity1: ScheduleActivity, activity2: ScheduleActivity): boolean {
    return activity1.predecessors?.some(p => p.activityId === activity2.id) || false;
  }
  ...
  private hasDependencyConflict(activity1: ScheduleActivity, activity2: ScheduleActivity): boolean {
    ...
    return pred1.some(p => p.activityId === activity2.id) ||
  ...
  private calculateResourceEfficiency(activities: ScheduleActivity[]): number {
    ...
    return activities.length > 0 ? 
      activities.filter(a => a.resourceRequirements?.workforce && a.resourceRequirements.workforce.length > 0).length / activities.length * 100 : 
      0;
  }
