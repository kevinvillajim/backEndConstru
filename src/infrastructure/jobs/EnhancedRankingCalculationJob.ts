...
export class EnhancedRankingCalculationJob {
	...
	private async notifyJobCompletion(result: JobExecutionResult): Promise<void> {
		try {
			// Solo notificar si hay algún problema o es un milestone importante
			if (!result.success || result.templatesProcessed > 100) {
				const notificationService = getNotificationService();

				const title = result.success
					? `✅ Job Completed: ${result.period}`
					: `⚠️ Job Completed with Issues: ${result.period}`;

				const content = result.success
					? `Job de rankings ${result.period} completado exitosamente. ${result.templatesProcessed} plantillas procesadas en ${result.executionTime}ms.`
					: `Job de rankings ${result.period} completado con errores. ${result.errors.length} errores encontrados.`;

				await notificationService.sendToUsersByRole("admin", {
					title,
					content,
					type: result.success ? ("SUCCESS" as any) : ("WARNING" as any),
					priority: result.success ? ("LOW" as any) : ("MEDIUM" as any),
					relatedEntityType: "system_job",
					relatedEntityId: result.jobId,
	...
	private async notifyJobFailure(jobName: string, error: any): Promise<void> {
		try {
			...
			// Como no tenemos una función notifyAdmins específica, usamos sendToUsersByRole
			await notificationService.sendToUsersByRole("admin", {
				title: `🚨 Job Failed: ${jobName}`,
				content: `El job de cálculo de rankings "${jobName}" ha fallado. Error: ${error.message || error}`,
				type: "SYSTEM" as any, // Usando el enum NotificationType.SYSTEM
				priority: "CRITICAL" as any, // Usando el enum NotificationPriority.CRITICAL
				relatedEntityType: "system_job",
				relatedEntityId: jobName,
				icon: "🚨",
				sendEmail: true, // Enviar por email para asegurar que los admins lo vean
				sendPush: true,
			});

			console.log(`✅ Job failure notification sent for ${jobName}`);
