...
export class EnhancedRankingCalculationJob {
  ...
  private initializeJobs(): void {
    ...
    this.scheduleJob('cleanup', this.config.dataRetention.cleanupSchedule, () => {
      ...
      const task = cron.schedule(
        ...
        async () => {
          if (this.isShuttingDown) {
            console.log(`⏹️  Skipping job ${jobName} - system is shutting down`);
            return;
          }

          const status = this.jobStatuses.get(jobName);
          if (status?.isRunning) {
            console.log(`⚠️  Job ${jobName} is already running, skipping this execution`);
            return;
          }
  ...
  private async executeCleanupJob(): Promise<JobExecutionResult> {
    ...
    try {
      ...
      console.log(`🧹 Cleanup completed: ${itemsProcessed} items removed in ${executionTime}ms`);
  ...
  public getMetrics() {
    ...
    return {
      ...
      lastExecutions: statuses
        ...
        .sort((a, b) => (b.lastExecution?.getTime() || 0) - (a.lastExecution?.getTime() || 0))
        .slice(0, 5)
    };
  }
