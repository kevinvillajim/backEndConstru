	...
	implements CalculationBudgetRepository
{
	...
	async create(
		budgetData: CreateCalculationBudgetDTO
	): Promise<CalculationBudget> {
	...
	async update(
		id: string,
		budgetData: Partial<CalculationBudget>
	): Promise<CalculationBudget | null> {
	...
	async save(budget: CalculationBudget): Promise<CalculationBudget> {
		...
		let budgetEntity: CalculationBudgetEntity;
		...
		if (existingBudget) {
			...
			Object.assign(existingBudget, {
				...
				materialsSubtotal: budget.materialsSubtotal,
				laborSubtotal: budget.laborSubtotal,
				professionalCostsTotal: budget.professionalCostsTotal,
				...
				customization: budget.customization,
				exportSettings: budget.exportSettings,
				lastCalculatedAt: budget.lastCalculatedAt,
				isTemplateBudget: budget.isTemplateBudget,
			...
			// Crear nueva entidad
			budgetEntity = this.toEntity(budget);
		}
	...
	async recalculateTotals(id: string): Promise<CalculationBudget | null> {
		...
		const taxAmount =
			(subtotal + contingencyAmount) * (budget.taxPercentage / 100);
