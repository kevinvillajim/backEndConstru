...
export class TypeOrmBudgetLineItemRepository implements BudgetLineItemRepository {
  ...
  async findByType(calculationBudgetId: string, itemType: LineItemType): Promise<BudgetLineItem[]> {
    const lineItems = await this.repository.find({
      where: { 
        calculationBudgetId,
        itemType
  ...
  async findBySource(calculationBudgetId: string, source: LineItemSource): Promise<BudgetLineItem[]> {
    const lineItems = await this.repository.find({
      where: { 
        calculationBudgetId,
        source
  ...
  async findByCalculationResult(sourceCalculationId: string): Promise<BudgetLineItem[]> {
    const lineItems = await this.repository.find({
  ...
  async createMany(lineItemsData: CreateBudgetLineItemDTO[]): Promise<BudgetLineItem[]> {
  ...
  async update(id: string, lineItemData: Partial<BudgetLineItem>): Promise<BudgetLineItem | null> {
  ...
  async updateQuantity(id: string, quantity: number, finalQuantity?: number): Promise<BudgetLineItem | null> {
    const lineItem = await this.repository.findOne({ where: { id } });
    if (!lineItem) return null;
  ...
  async updatePrice(id: string, unitPrice: number): Promise<BudgetLineItem | null> {
    const lineItem = await this.repository.findOne({ where: { id } });
    if (!lineItem) return null;
  ...
  async getTotalByCategory(calculationBudgetId: string, category: string): Promise<number> {
    const result = await this.repository
      .createQueryBuilder("lineItem")
      .select("SUM(lineItem.subtotal)", "total")
      .where("lineItem.calculationBudgetId = :calculationBudgetId", { calculationBudgetId })
  ...
  async getTotalByType(calculationBudgetId: string, itemType: LineItemType): Promise<number> {
    const result = await this.repository
      .createQueryBuilder("lineItem")
      .select("SUM(lineItem.subtotal)", "total")
      .where("lineItem.calculationBudgetId = :calculationBudgetId", { calculationBudgetId })
