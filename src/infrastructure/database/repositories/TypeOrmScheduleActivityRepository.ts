...
// ✅ CORRIGIDO: Importar entidad desde infrastructure y enums desde domain
import {ScheduleActivityEntity} from "../entities/ScheduleActivityEntity";
import {
	ActivityStatus,
	ConstructionTrade,
...
export class TypeOrmScheduleActivityRepository
	implements ScheduleActivityRepository
{
	...
	async findResourceConflicts(dateRange: {
		start: Date;
		end: Date;
	}): Promise<any[]> {
		return await this.repository
			...
			.where("activity.plannedStartDate >= :startDate", {
	...
	async getProductivityMetrics(
		...
		if (dateRange) {
			query = query
				.andWhere("activity.plannedStartDate >= :startDate", {
					startDate: dateRange.start,
				})
				.andWhere("activity.plannedEndDate <= :endDate", {
					endDate: dateRange.end,
				});
		}

		return await query.groupBy("activity.primaryTrade").getRawMany();
	}
	...
	private calculateCriticalPath(
		...
		return activities.filter((activity) => {
			...
			const duration =
				activity.plannedEndDate.getTime() - activity.plannedStartDate.getTime();
