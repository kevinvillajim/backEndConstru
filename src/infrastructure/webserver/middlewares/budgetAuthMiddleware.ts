...
export class BudgetAuthMiddleware {
	...
	checkBudgetPermissions(requiredPermissions: Array<keyof BudgetPermissions>) {
		return async (
			...
			try {
				...
				const budget =
					await this.calculationBudgetRepository.findById(budgetId);
				if (!budget) {
					res.status(404).json({
						success: false,
				...
				const permissions = await this.getBudgetPermissions(
					budget,
					req.user.id
				);
	...
	checkTemplatePermissions(requiredActions: string[]) {
		return async (
			...
			try {
				...
				const template =
					await this.budgetTemplateRepository.findById(templateId);
				if (!template) {
					res.status(404).json({
						success: false,
				...
				const hasPermission = await this.checkTemplateAccess(
					template,
					req.user.id,
					requiredActions
				);
	...
	checkProjectAccess() {
		return async (
			...
			try {
				...
				if (!projectId) {
					next(); // Si no hay projectId en el body, continuar
					return;
				}
				...
				if (!hasAccess) {
					res.status(403).json({
						success: false,
						message:
							"No tiene permisos para crear presupuestos en este proyecto",
					});
					return;
				}
	 ...
	 */
	checkPlanLimits(
		action: "CREATE_BUDGET" | "CREATE_TEMPLATE" | "EXPORT_DOCUMENT"
	) {
		return async (
			req: RequestWithUser,
			res: Response,
	...
	private async getBudgetPermissions(
		budget: any,
		userId: string
		...
		if (budget.status === CalculationBudgetStatus.FINAL) {
			...
			restrictions.push(
				"Los presupuestos finales no pueden modificarse o eliminarse"
			);
		}
		...
		if (budget.status === CalculationBudgetStatus.APPROVED) {
			permissions.canEdit = false;
			restrictions.push(
				"Los presupuestos aprobados requieren crear una nueva versión para modificar"
			);
		}
	...
	private async checkTemplateAccess(
		...
		if (template.scope === TemplateScope.SYSTEM) {
			...
			if (
				requiredActions.some((action) => ["EDIT", "DELETE"].includes(action))
			) {
				return {
					allowed: false,
			...
			if (
				...
				if (!isCreator && !isCompanyAdmin) {
					return {
						allowed: false,
						reason:
							"Solo el creador o administradores pueden modificar este template",
					};
				}
			}
		...
		if (template.scope === TemplateScope.SHARED) {
			const hasSharedAccess = await this.checkSharedTemplateAccess(
				template.id,
				userId
			);
			if (!hasSharedAccess) {
	...
	private async getUserCurrentUsage(userId: string) {
		...
		const budgetsThisMonth =
			await this.calculationBudgetRepository.countByUserAndMonth(
				userId,
				currentMonth
			);
		const personalTemplates =
			await this.budgetTemplateRepository.countPersonalByUser(userId);
	...
	private getPermissionDescription(
		permission: keyof BudgetPermissions
	): string {
		const descriptions = {
			canRead: "ver",
			canEdit: "editar",
	...
	private async checkSharedTemplateAccess(
		templateId: string,
		userId: string
	): Promise<boolean> {
